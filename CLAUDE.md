...
- Face-swap — put YOUR real face into generated photos/videos (local ReActor/InsightFace in ComfyUI). The persona ("You as") gains a `faceRef` photo (`PersonaModal`); `lib/face.js` `registerFace` POSTs it once to `/api/face` (cached → short id stored under `data/faces/<id>.png`). With Settings → Photos → "Put my face in scenes" on (`settings.faceSwap`), `generatePhoto` wraps the still through `/api/faceswap?image=…&face=<id>` and `generateVideo` passes `&face=<id>` to `/api/img2vid` (swap the still, then animate). Backend `_comfy_faceswap` runs the `ReActorFaceSwap` node (`inswapper_128.onnx` + `GFPGANv1.4.pth` restore). Verified end-to-end. See [[comfyui-local-video]].
