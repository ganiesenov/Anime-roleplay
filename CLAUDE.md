...
- **New** `frontend-next/` — **Vite 6 + React 18 + Tailwind v4** (CSS-first, theme tokens in `src/index.css` `@theme{}`, no tailwind.config). Plain JSX (no TS). `base: '/next/'`. Served from `frontend-next/dist` — **the backend serves the BUILT output, so you must `npm run build` after changing `frontend-next/src`.**
  ...
  - `ChatView.jsx` is the chat container (state + flows); its view layer is split out into `ChatMessage.jsx` (the `MessageBubble` + photo), `icons.jsx` (glyphs + `CtrlBtn`/`Meter`), and `lib/media.js` (`avatarUrl`/`isVideoUrl`). Peripheral state is extracted into hooks (`hooks/useMusic`, `useTts`, `useSuggestions`); the core stream/relationship/presence/group logic + the `force()`-rerender → `useReducer` swap is the next refactor step.
