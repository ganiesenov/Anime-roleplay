...
- Per-message "🎞 animate this moment" — alongside "🖼 illustrate this moment", AI replies get a Film button that renders a short scene-aware clip of that exact beat. It reuses the selfie likeness pipeline (cached `char.appearance` + `tagsFromScene`) and routes the still through ComfyUI + Stable Video Diffusion via `buildVideoUrl` → `/api/img2vid` (animated WebP, plays in the same photo bubble). `generateVideo()` mirrors `generatePhoto()`; `animateMessage()` mirrors `illustrateMessage()`. The manual `/video <tags>` command now shares `generateVideo` so it gets the same likeness. Comfy-only (needs an SVD checkpoint): the button only shows when the photo provider is ComfyUI. A `loadingKind:'video'` flag on the variant (read by `getMessageLoadingKind`) drives the "animating clip…" copy and a video-specific error hint.
