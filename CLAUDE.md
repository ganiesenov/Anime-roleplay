...
- Face-swap / "scene with me" — put YOUR real face into generated photos/videos as a realistic two-person scene (you + the character), via local ReActor/InsightFace in ComfyUI. The persona ("You as") gains a `faceRef` photo + `gender` (`PersonaModal`); `lib/face.js` `registerFace` POSTs the face once to `/api/face` (cached → `data/faces/<id>.png`). With Settings → Photos → "Put my face in scenes" on (`settings.faceSwap`) AND a persona face, `generatePhoto`/`generateVideo` switch to a "with me" path: `aigen.sceneWithUser` writes a realistic two-person prompt (you, from persona desc/gender, + the character rendered real) for the moment (chat transcript) or your explicit request; the still is generated raw-realistic, then ReActor swaps your face onto your gender-matched body (`detect_gender_input`), and for video SVD animates it. Endpoints: `/api/faceswap?image=&face=&gender=` and `/api/img2vid?…&face=&faceGender=` (`_comfy_faceswap`, `ReActorFaceSwap` + `inswapper_128.onnx` + `GFPGANv1.4.pth`; graceful fallback to the un-swapped scene if no face is found). Verified end-to-end. See [[comfyui-local-video]].
